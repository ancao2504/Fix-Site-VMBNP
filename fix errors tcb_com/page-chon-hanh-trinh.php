...
elseif (isset($_GET["SessionID"]) && !isset($_POST['dep']) && !isset($_POST['sm_request'])) {
  ...
  $countactive = ((isset($condition['active']['vna'])) ? 1 : 0) + ((isset($condition['active']['vj'])) ? 1 : 0) + ((isset($condition['active']['js'])) ? 1 : 0) + ((isset($condition['active']['qh'])) ? 1 : 0) + ((isset($condition['active']['sabre'])) ? 1 : 0);
  // echo '<pre>';
  //   print_r( $countactive );
  // echo '</pre>';
  $arrlinkrs = array();
...
<div class="row" id="colLeftNoBorder">
  <div class="block">
    ...
    if ($condition) {
      ...
      if(isset($geoip_country_code) && $geoip_country_code) {
        $geoip_country_code = getenv(GEOIP_COUNTRY_CODE); // fix php7
      }
      ...
      if (!$ip_address || strtoupper($ip_address) == 'UNKNOWN' || (checkCIDRBlacklist($ip_address) && !$_SESSION['fl_captcha_ok']) || ($req_count > $req_count_allow && !isset($_SESSION['fl_captcha_ok']))) {
        ...
        // if (!$condition["isinter"]) {
          ...
          $waiting_notices = '<div class="col-md-8 col-md-offset-2 col-sm-12 waiting_block"><h2>Khởi hành từ <span class="fontplace">' . $source_ia . '</span> đi <span class="fontplace">' . $destination_ia . '</span></h2><table><tr><td>Loại vé:</td><td><strong>' . $GLOBALS['way_flight_list'][$condition['way_flight']] . '</strong></td><td>Số hành khách:</td><td><strong>' . $condition['adult'] . ' người lớn' . $qty_children . $qty_infants . '</strong></td></tr><tr><td>Ngày khởi hành:</td><td><strong>' . $condition['depart'] . '</strong></td><td>' . $str_return . '</td></tr></table><p class="notice-waiting">Mời bạn vui lòng chờ trong giây lát ...</p></div>';
          ...
          if(!$condition['active']['vna'] 
            ...
            <script type="text/javascript">
              ...
              var checkInter = <?php echo ($condition['isinter'] ? 1 : 0); ?>;
                ...
                var SourceCity = '<?php echo $source_ia?>';
								var DesCity = '<?php echo $destination_ia?>';
              ...
              for (var i = 0; i < Getrs.length; i++) {
                XhrRequest[i] = $.ajax({
                  ...
                  traditional: true,
                  type: "POST",
                  data: "enCode=" + SessionID + "&cache=<?php if(isset($_GET["clearcache"])) echo ($_GET["clearcache"]) ? 0 : 1; ?>&<?php if($_SESSION['fl_token']) echo $_SESSION['fl_token']; ?>=",
                  timeout: 45000,
                  dataType: "html"
                }).done(function(data) {
                  $(function() {
    ...
    <div class="col-md-6 col-md-offset-3 col-sm-12">
      <?php if (isset($isempty_flight) && !empty($isempty_flight)) {
    ...
    <div id="mainDisplay" class="col-md-8 sidebar-separator">
      <div id="result">
        <form action="<?php echo _page("passenger") ?>" method="post" name="frmSelectFlight" id="frmSelectFlight">
          ...
          <?php if($condition["isinter"]) { ?>
              <div class="searchresults">
                <div style="position: relative;" class="lld-result list-search-result">
                  ...
                  <!-- ONE-WAYS-->
                  <?php if(isset($_SESSION['search']['way_flight']) && $_SESSION['search']['way_flight'] == "1") { ?>
                  <div class="row sinfo box margin-bottom-info">
                    <div class="headerBlock col-md-12" style="padding-left: 0px; padding-right: 0px; margin-bottom: 0px; margin-top: 0px;">
                      <div class="headerArea">
                        <div class="row">
                          ...
                          <div class="col-xs-6">
                            <span style="float:right;text-align:right;" id="date-dep">Ngày đi: <?php echo $condition['depart']; ?></span>
                          </div>
                        </div>
                      </div>
                      <div class="location contload"></div>
                    </div>
                  </div>
                  <?php } ?>
                  <!-- END ONE-WAYS -->     
                      ...
                      <div class="headerBlock col-md-12" style=" padding-left: 0px; padding-right: 0px;margin-bottom: 0px;margin-top: 0px;">
                        <div class="headerArea">
                          <div class="row">
                            <div class="col-md-6">
                              <div class="stepHeader stepInstruction">
                                <span class="big big-inter"><?php echo $source_ia . "  &nbsp;&nbsp;&nbsp;<i class='fa fa-exchange'></i> &nbsp;&nbsp;&nbsp; " . $destination_ia ?></span></div>
                            </div>
                            <div class="col-xs-6 class-date-roundtrips">
                              <?php if ($condition['way_flight'] == 0 && $condition['return'] != '') {
                                  echo "<span class='date-dep-inter' id='date-dep'> Ngày đi: ". $condition['depart'] ." </span>";
                                  echo "<span class='data-ret-inter' id='date-ret'> Ngày về: ". $condition['return'] . " </span>";
                                }
                              ?>
                            </div>
                          </div>
          ...
          <?php if (isset($_SESSION['search']['way_flight']) && $_SESSION['search']['way_flight'] == "0" && !$condition['isinter']) { ?>
            <!--Thong Tin Chang Di-->
            <div class="row sinfo box">
              <div class="headerBlock col-md-12" style=" padding-left: 0px; padding-right: 0px;margin-bottom: 0px;margin-top: 0px;">
                <div class="headerArea">
